...
function startCompliments(durationSeconds = 20) {
    ...
    // Track the current compliment index
    let currentIndex = 0;
    const intervalMs = 2500;
    
    complimentsInterval = setInterval(() => {
        // Check if there are still compliments left in the list
        if (currentIndex < complimentsList.length) {
            ...
            // Stop the timer once the last compliment is shown
            clearInterval(complimentsInterval);
            complimentsInterval = null;
...
function complimentScroll() {
    // Scrolls the specific container instead of the whole body
...
function startTypingLetter() {
    ...
    const timer = setInterval(() => {
        ...
        i++;
        // Keeps the typing visible on phone screens
